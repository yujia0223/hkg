...
class NCRPNode(object):
    ...
    def __init__(self, num_levels, vocab, tail_voacb, parent=None, level=0,
                 random_state=None):
        ...
        self.tail_vocab = np.array(tail_voacb)
        self.word_counts = np.zeros(len(vocab))
        self.tail_counts = np.zeros((len(vocab),len(tail_voacb)))
    ...
    def add_child(self):
        ...
        node = NCRPNode(self.num_levels, self.vocab,self.tail_vocab, parent=self, level=self.level+1)
...
class HierarchicalLDA(object):
    ...
    def __init__(self, corpus, tail_corpus, vocab, tail_vocab,
                 alpha=10.0, gamma=1.0, eta=0.1, eta_tail = 1,
        ...
        self.corpus = corpus
        self.tail_corpus = tail_corpus
        self.vocab = vocab
        self.tail_vocab = tail_vocab
        ...
        self.tail_num_types = len(tail_vocab)
        self.eta_tail_sum = eta_tail * self.tail_num_types
        
        self.results = []
        self.results_customers = []
        self.labels = []
        self.allnodes_dict = {}
        ...
        # we added earlier.
        self.root_node = NCRPNode(self.num_levels, self.vocab, self.tail_vocab)
        ...
        for d in range(len(self.corpus)):
            ...
            for n in range(doc_len):
                ...
                random_node.tail_counts[w,t_w] += 1
    ...
    def estimate(self, num_samples, display_topics=50, n_words=5, with_weights=True):
        ...
        for s in range(num_samples):
            ...
            if (s > 0) and ((s+1) % display_topics == 0):
                ...
                print
        self.save_results(n_words, with_weights)
        # tree_file = open("tree.txt", "w")
        # n = tree_file.write(self.tree)
        # tree_file.close()        
    ...
    def sample_path(self, d):
        ...
        for n in range(len(doc)): # for each word in the doc
            ...
            level_node.tail_counts[w,t_w] -=1
            level_node.total_words -= 1
            assert level_node.word_counts[w] >= 0
            assert level_node.total_words >= 0
            assert level_node.tail_counts[w,t_w] >=0

        self.calculate_doc_likelihood(node_weights, level_word_counts, doc_levels, doc, tail_doc)
        ...
        for level in range(self.num_levels-1, -1, -1): # e.g. [3, 2, 1, 0] for num_levels = 4
            ...
            level_index = np.where(doc_levels==level)[0]
            for m in level_index:
                w = doc[m]
                t_w = tail_doc[m]
                node.tail_counts[w, t_w] += 1
            node = node.parent
    ...
    def calculate_doc_likelihood(self, node_weights, level_word_counts,doc_levels,doc,tail_doc):
        ...
        for level in range(1, self.num_levels):  # skip the root
            ...
            for w in word_counts:
                count = word_counts[w]
                for i in range(count):  # why because gamma function hahaha
                    new_topic_weights[level] += log((self.eta + i) / (self.eta_sum + total_tokens))
                    total_tokens += 1
                    
            tail_count = np.where(doc_levels==level)[0]
            tail_total_words = np.zeros(len(self.tail_vocab))
            for index in tail_count:
                word_index = doc[index]
                new_topic_weights[level] += log( (self.eta_tail + 1) /
                                (self.eta_tail_sum + tail_total_words[word_index]) )
                tail_total_words[word_index] += 1

        self.calculate_word_likelihood(node_weights, self.root_node, 0.0, level_word_counts, new_topic_weights, 0,doc_levels,doc,tail_doc)

    def calculate_word_likelihood(self, node_weights, node, weight, level_word_counts, new_topic_weights, level,doc_levels,doc,tail_doc):
        ...
        for w in word_counts:
            count = word_counts[w]
            for i in range(count): # why ????????? still function
        ...
        for index in tail_count:
            word_index = doc[index]
            tail_index = tail_doc[index]
            node_weight += log( (self.eta_tail + node.tail_counts[word_index,tail_index] + 1) /
                                (self.eta_tail_sum + np.sum(node.tail_counts[word_index]) + 
                                 tail_total_words[word_index]) )
            tail_total_words[word_index] += 1
        ...
        for child in node.children:
            self.calculate_word_likelihood(node_weights, child, weight + node_weight,
                                           level_word_counts, new_topic_weights, level+1,doc_levels,doc,tail_doc)
        ...
        for n in range(len(doc)):
            ...
            for level in range(self.num_levels):
                level_weights[level] = (self.alpha + level_counts[level]) *                     \
                    ...
                    (self.eta_tail + path[level].tail_counts[w,w_t]) /                          \
                    (self.eta_tail_sum + np.sum(path[level].tail_counts[w]))
    ...
    def save_results(self, n_words, with_weights):
        self.save_result(self.root_node, n_words, with_weights)

    def save_result(self, node, n_words, with_weights):
        out = node.get_top_words(n_words, with_weights)
        p_out = out
        self.labels.append((node.node_id, node.customers))
        self.allnodes_dict[node.node_id]=node
        for child in node.children:
            self.results_customers.append((node.node_id, child.node_id))
            c_out = child.get_top_words(n_words, with_weights)
            self.results.append((p_out,c_out))
            self.save_result(child, n_words, with_weights)
